...
# Additional imports for randomization and TCP health checks
import random
import socket
...
# Priority ports receive higher bonus to surface common WS ports first
...
# Country-specific TLDs that should be given higher priority (USA, Germany, Netherlands, France)
...
def safe_port(p: urllib.parse.SplitResult) -> int:
    """
    Returns an integer port from urlsplit SplitResult.
    Falls back to parsing digits from netloc if urlsplit.port contains extra characters.
    """
    try:
...
def is_port_open(host: str, port: int, timeout: float = 3.0) -> bool:
    """Attempt a TCP connection to ``host:port`` to verify the endpoint is reachable.

    Returns ``True`` if a TCP connection can be established within ``timeout`` seconds,
    otherwise returns ``False``. Ports that are invalid or hosts that cannot be
    resolved will return ``False``.
    """
    if not host or port <= 0:
        return False
    try:
        # ``socket.create_connection`` automatically resolves DNS and connects
...
def _port_bonus(port: int) -> int:
    """Return a bonus based on the port. Ports in ``PORT_PRIORITY`` get a larger bonus.
    Ports that are invalid return a penalty. Ports in ``GOOD_PORTS`` get a small bonus."""
    if port <= 0:
        return -50  # invalid only
    # Large bonus for priority ports
    if port in PORT_PRIORITY:
        return PORT_PRIORITY[port]
    # Moderate bonus for other known good ports
...
def _vmess_key_score(config: str) -> Optional[ScoredConfig]:
    ...
    # Drop endpoints that fail a basic TCP health check to reduce broken proxies
    if not is_port_open(add, port):
        return None

    score = 18  # baseline vmess
    # Prefer websocket transports and common WS ports
    if net == "ws":
        score += 5
    ...
    # Prioritize hosts in specific countries based on TLD
...
def _vless_key_score(config: str) -> Optional[ScoredConfig]:
    ...
    # Drop endpoints that fail a basic TCP health check
...
def _hy2_key_score(config: str) -> Optional[ScoredConfig]:
    ...
    # Prefer websocket-like types if indicated by query; hy2 has no explicit transport
    # but we still apply port bonus and country bonus.
    score += _port_bonus(port)
    # Prioritize hosts in specific countries based on TLD
    if any(host.endswith(tld) for tld in COUNTRY_TLDS):
        score += 5
...
def main() -> None:
    ...
    # Sort by score descending to take top performers first
    scored.sort(key=lambda x: x.score, reverse=True)
    scored = scored[:MAX_OUTPUT]
    # Shuffle the final list to intermix vmess and vless entries for variety
